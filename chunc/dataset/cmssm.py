...
class cMSSMDataset(MSSMDataset):
    """
    ...
    def __init__(self,
        ...
        super(cMSSMDataset, self).__init__(input_dir)
        self.subspace = 'cmssm'
    ...
    def generate_training_set(self,
        ...
        meta_dict = {
            ...
            "features": {
                key:ii for ii, key in enumerate(cmssm_columns)
            },
            "classes": {
                "valid": 0
            },
    ...
    def generate_unconstrained_testing_batches(self,
        unconstrained_file: str,
        num_files:          int=10,
        save:               bool=True,
        output_file:        str='',
        ...
        num_events = len(unconstrained_dataframe)
        step_size = int(num_events / num_files)
        for ii in range(num_files):
            start_idx = ii * step_size
            end_idx = (ii + 1) * step_size
            unconstrained_events = unconstrained_dataframe[start_idx:end_idx].values
            unconstrained_labels = np.zeros(len(unconstrained_events)).reshape(-1,1)
            ...
            meta_dict = {
                ...
                "when_created": datetime.now().strftime("%m-%d-%Y-%H:%M:%S"),
                "where_created": socket.gethostname(),
                "num_events": len(unconstrained_events),
                "features": {
                    key:ii for ii, key in enumerate(cmssm_columns)
                },
                "classes": {
            ...
            if save:
                np.savez(
                    "datasets/"+output_file+f"_{ii}.npz",
                    meta=meta_dict,
                    event_features=unconstrained_events,
                    event_classes=unconstrained_labels,
                )
    ...
    def load_constrained_set(self,
        ...
        if not os.path.isfile(constrained_file):
            self.logger.error(f"Specified constrained file '{constrained_file}' not in path.")
            raise ValueError(f"Specified constrained file '{constrained_file}' not in path.")
        if labeling not in ['cmssm']:
            self.logger.warning(f"Specified labeling '{labeling}' not allowed, using 'cmssm'")
            labeling = 'cmssm'
        # load files
        constrained_dataframe = pd.read_csv(
            constrained_file, 
