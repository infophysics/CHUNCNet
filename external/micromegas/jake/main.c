...
//  printf("Cold Dark Matter Candidate:  %s\n", cdmName);
//  printf("Mcdm=%E\n", Mcdm);   
...
#ifdef OMEGA
{ int fast=1;
  //double Beps=1e-5, cut=.01;
  double Beps=1e-4, cut=.01;
...
// to include processes with virtual W/Z  also  in co-annihilation 
  ...
  //printf("\n==== Calculation of relic density =====\n");  
   ...
   if(Omega>0) nPrc = printChannels(Xf,cut,Beps,1,NULL);
...
#endif 
FILE* fp;
int argvlen=strlen(argv[2]) + strlen(argv[3]);
char saveFile[argvlen + 10];
char* saveThread;
strcpy(saveFile, argv[2]);
saveThread=argv[3];
strcat(saveFile,"/micrOmegas_output");
strcat(saveFile, saveThread);
strcat(saveFile,".txt");
fp = fopen(saveFile,"w");
...
fprintf(fp, "%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g", 
          Omega, gmuon(), bsgnlo(&SMbsg), SMbsg, btaunu(), bsmumu(), dtaunu(&dmunu), dmunu, deltarho(), Rl23(), Mcdm, sigmaV, SCcoeff*pA0[0]*pA0[0],3*SCcoeff*pA5[0]*pA5[0], SCcoeff*nA0[0]*nA0[0],3*SCcoeff*nA5[0]*nA5[0]);
// DM Channels
int i;
/*
for(i=0; i<nPrc;i++) 
    fprintf(stdout, "%i  %.4E   %s %s -> %s %s\n", i, omegaCh[i].weight, omegaCh[i].prtcl[0],
  omegaCh[i].prtcl[1],omegaCh[i].prtcl[2],omegaCh[i].prtcl[3]);
*/
if(nPrc >= 10)
    for(i=0; i<10;i++)
        fprintf(fp, ",%.4E,%s,%s,%s,%s", omegaCh[i].weight, omegaCh[i].prtcl[0],
        omegaCh[i].prtcl[1],omegaCh[i].prtcl[2],omegaCh[i].prtcl[3]);
if(nPrc< 10)
{
    for(i=0;i<nPrc;i++)
